    ...
    async def run_analysis(email: Email) -> EvidenceReport:
        ...
        for name, (success, evidence, err_msg) in zip(analyzer_names, results):
            ...
            if success:
                ...
                for ev in evidence:
                    triggered_rules.append(ev.triggered_rule)
            else:
                ...
                triggered_rules.append("GEN_ERR")
                
        scan_duration_ms = (time.perf_counter() - start_time) * 1000.0
        
        # Calculate summary statistics
        total_risk = sum(ev.risk_contribution for ev in evidence_list)
        high_severity_count = sum(1 for ev in evidence_list if ev.severity in ["HIGH", "CRITICAL"])
        
        confidence_summary = {
            "overall_threat_weight": total_risk,
            "high_severity_alerts": high_severity_count,
            "average_confidence": sum(ev.confidence for ev in evidence_list) / len(evidence_list) if evidence_list else 1.0
        }
